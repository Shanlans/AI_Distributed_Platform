...
from DeepLabV3_plus_official import feature_extractor, deeplabV3_plus_model_option, train_utils
...
def multi_scale_logits(images,
    ...
    # image_pyramid: Input image scales for multi-scale feature extraction.
    if not model_options.image_pyramid:
        image_pyramid = [1.0]
    else:
        image_pyramid = model_options.image_pyramid
...
def build_deeplabv3_plus(images,
                         model_options,
                         weight_decay=0.0001,
                         is_training=False,
                         fine_tune_batch_norm=False):
    ...
    # Format like this "{'merged_logits': <tf.Tensor 'ResizeBilinear_2:0' shape=(2, 128, 128, 5) dtype=float32>}""
    output_type_dict = outputs_to_scales_to_logits[deeplabV3_plus_model_option.OUTPUT_TYPE]
    logits = output_type_dict[get_merged_logits_scope()]
    logits = tf.identity(logits, name=deeplabV3_plus_model_option.OUTPUT_TYPE)
