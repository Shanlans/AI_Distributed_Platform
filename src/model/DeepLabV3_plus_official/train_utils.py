...
def add_softmax_cross_entropy_loss_for_each_scale(scales_to_logits,
  ...
  """
  if not num_classes == len(loss_weight):
    raise ValueError('Class weights number should match class number')
  if labels is None:
  ...
  for scale, logits in six.iteritems(scales_to_logits):
    ...
    for i in range(num_classes):
      weights+= tf.to_float(tf.equal(scaled_labels,i))*loss_weight[i]
