...
def preprocess_image(image_path,label_path, is_training):
    image_string = tf.read_file(image_path)
    image = tf.image.decode_image(image_string)
    image = tf.image.grayscale_to_rgb(image)
    image = tf.image.resize_nearest_neighbor(tf.expand_dims(image,0), [_HEIGHT,_WIDTH], align_corners=True)
    image = tf.to_float(tf.image.convert_image_dtype(tf.squeeze(image,axis=0), dtype=tf.uint8))
    image.set_shape([None, None, 3])

    label_string = tf.read_file(label_path)
    label = tf.image.decode_png(label_string, 1)
    label = tf.image.resize_nearest_neighbor(tf.expand_dims(label,0), [_HEIGHT,_WIDTH], align_corners=True)
    label = tf.to_int32(tf.image.convert_image_dtype(tf.squeeze(label,axis=0), dtype=tf.uint8))
